...
class AkkaHttpServer(serverConfig: ServerConfig, db: PostgresClient)(
  ...
  def run(): Future[Http.ServerBinding] = {
    ...
    val service: HttpRequest => Future[HttpResponse] = SustainabilityScoresServiceHandler(
      new SustainabilityScoreServiceImpl(db))

    val binding = Http().newServerAt(serverConfig.host, serverConfig.port).bind(service)

    // report successful binding
    binding.foreach { binding =>
      info(s"gRPC server bound to: ${binding.localAddress} / ${serverConfig.host}:${serverConfig.port}")
    }
    binding
  }
}
